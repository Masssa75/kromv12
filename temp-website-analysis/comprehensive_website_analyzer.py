    ...
    def parse_website_with_playwright(self, url):
        ...
        try:
            with sync_playwright() as p:
                ...
                # Many sites never reach networkidle due to continuous analytics/ads
                page.goto(url, wait_until='domcontentloaded', timeout=15000)
                
                # Smart content waiting - check content and retry if too minimal
                max_attempts = 3
                for attempt in range(max_attempts):
                    ...
                    current_content = page.evaluate("() => document.body ? document.body.innerText : ''")
                    content_length = len(current_content)
                    
                    # If we have substantial content, we're done
                    if content_length > 500:
                        break
                    
                    # If content is very minimal (loading screens typically < 100 chars)
                    if content_length < 100 and attempt < max_attempts - 1:
                        print(f"     ⏳ Minimal content ({content_length} chars), waiting for loading to complete...")
                        # Wait longer for loading screens to finish
                        page.wait_for_timeout(3000)
                        
                        # Also try waiting for content to grow
                        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if json_match:
                        ...
                        print(f"  ✅ Score: {analysis.get('total_score')}/21")
                        print(f"  👥 Team members found: {len(analysis.get('exceptional_signals', []))}")
...
ALL NAVIGATION LINKS ({len(all_links)} total):"""
        ...
        other_links = [l for l in all_links if l.get('priority') not in ['high', 'medium']]
        
        if high_priority:
            prompt += "\nHIGH PRIORITY:"
            for link in high_priority[:15]:
                prompt += f"\n- [{link.get('type', '').upper()}] {link.get('text')}: {link.get('url')}"
        
        if medium_priority:
            prompt += "\nMEDIUM PRIORITY:"
            for link in medium_priority[:10]:
                prompt += f"\n- [{link.get('type', '').upper()}] {link.get('text')}: {link.get('url')}"
        
        if other_links:
            prompt += f"\nOTHER LINKS ({len(other_links)} total):"
...
EXTRAORDINARY ACHIEVEMENTS (CRITICAL - READ CAREFULLY):
IMPORTANT: Look through ALL the content for any extraordinary achievements, even if not in traditional format.
Search for impressive numbers, metrics, or backgrounds mentioned ANYWHERE in the text, including:

- Revenue/financial metrics: Look for "$" followed by numbers (e.g., "$50 million", "$10M revenue")
- User/audience metrics: Look for "subscribers", "users", "followers", "views" with large numbers
- Team achievements: Look for team member names followed by achievements (e.g., "Kong... 4 million subscribers")
- Founder credentials: "YC-backed", "ex-Google", "ex-Facebook", "Forbes 30 under 30", etc.
- Growth metrics: "75M users in 2 months", "10x growth", "fastest growing", etc.
- Business success: "profitable", "break-even", "customers include", "partnered with", etc.
- Technical achievements: "patent", "breakthrough", "first to", "only platform that", etc.

CRITICAL: Even if team info isn't in a formal "Team" section, if you see names with impressive achievements 
(like "Kong... has grown multiple YouTube channels with a total of over 4 million subscribers"), 
that's an exceptional signal! List it in exceptional_signals AND give credit in team_transparency.

If you find extraordinary achievements, you MUST list them in "exceptional_signals" and adjust category scores accordingly.

STAGE 2 THRESHOLD: Projects scoring 10+ total points (out of 21) OR having automatic qualifiers warrant deeper investigation.

AUTOMATIC STAGE 2 QUALIFIERS (override score threshold):
- Has comprehensive documentation (docs.*, gitbook, wiki, etc.)
- Has installable applications (Download links, App Store, Google Play)
- Has GitHub repository with actual code
- Has published research papers or technical whitepapers
- Has verifiable partnerships with major companies
- Has audit reports from recognized firms

Provide your analysis as JSON:
{
  "total_score": (sum of all 7 categories, max 21),
  "automatic_stage_2_qualifiers": ["list any automatic qualifiers found, e.g., 'Has documentation portal', 'Has GitHub repository', 'Has mobile apps'"],
  "proceed_to_stage_2": (true if total_score >= 10 OR automatic_stage_2_qualifiers has items, false otherwise),
  ...
  "stage_2_links": [
    "Select 3-7 most valuable URLs from ALL NAVIGATION LINKS above for Stage 2 deep analysis",
    "Choose links that would provide the most insight about the project's legitimacy and potential",
    "Include docs, GitHub, whitepapers, roadmaps, team pages, tokenomics, partnerships, etc.",
    "Use the EXACT URLs from the navigation links above",
...
Return ONLY the JSON, no other text."""
    ...
    def save_to_database(self, url, parsed_data, ai_analyses):
        ...
        if ai_analyses:
            ...
            auto_qualifiers = first_analysis.get('automatic_stage_2_qualifiers', [])
            proceed_to_stage_2 = first_analysis.get('proceed_to_stage_2', False)
            if not proceed_to_stage_2 and auto_qualifiers:
            ...
            cursor.execute("""
                ...
                (ticker, url, parsed_content, documents_found, team_members_found, 
                 ...
                 missing_elements, quick_assessment, stage_2_links, automatic_stage_2_qualifiers)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ticker,
                url,
