...
def save_analysis(ticker, url, network, analysis_result):
    ...
    if analysis_result:
        ...
        cursor.execute('''
            ...
            (ticker, url, network, total_score, tier, proceed_to_stage_2, signals, summary, category_scores, analyzed_at, content_fetched)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            ticker,
            url,
...
def main():
    ...
    for i, token in enumerate(tokens[:20], 1):  # Start with 20 for testing
        ticker = token['symbol']
