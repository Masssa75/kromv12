#!/usr/bin/env python3
"""
Dashboard API server for the clean 'calls' table structure.
Serves both the HTML interface and API endpoints.
"""
...
def get_stats():
    ...
    try:
        with db_lock:
            ...
            cursor.execute("SELECT COUNT(*) as calls_with_raw_data FROM calls WHERE raw_data IS NOT NULL AND raw_data != ''")
...
def get_calls():
    ...
    try:
        ...
        with db_lock:
            ...
            # Simple query - all data is in one table now
            base_query = """
                SELECT 
                    *,
                    CASE 
                        WHEN timestamp IS NOT NULL THEN datetime(timestamp, 'unixepoch')
                        ELSE call_timestamp 
                    END as formatted_timestamp,
                    CASE 
                        WHEN buy_timestamp IS NOT NULL THEN datetime(buy_timestamp / 1000, 'unixepoch')
                        ELSE NULL 
                    END as formatted_buy_timestamp,
                    CASE 
                        WHEN top_timestamp IS NOT NULL THEN datetime(top_timestamp / 1000, 'unixepoch')
                        ELSE NULL 
            ...
            if search:
                where_clause = "WHERE ticker LIKE ? OR name LIKE ? OR message LIKE ? OR group_name LIKE ?"
...
def get_groups():
    ...
    try:
        with db_lock:
            ...
            # Get groups directly from calls table
            cursor.execute("""
                SELECT 
                    group_name,
...
def test():
    ...
    return jsonify({
        'status': 'ok',
        'message': 'Dashboard API server is running (clean database)',
        'table': 'calls',
        'endpoints': [
            '/api/stats',
            '/api/calls',
...
if __name__ == '__main__':
    print("=" * 60)
    print("KROM Dashboard API Server - Clean Database")
    print("=" * 60)
    print(f"Database: {DB_PATH}")
    print("Using table: 'calls'")
