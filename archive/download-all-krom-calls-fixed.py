#!/usr/bin/env python3
"""
Download ALL KROM calls from the API using proper pagination
Fixed version using offset parameter
"""
...
def get_krom_calls_batch(offset=0):
    """Fetch a batch of KROM calls from the API using offset"""
    if not KROM_API_TOKEN:
        raise ValueError("KROM_API_TOKEN not found in environment")
    
    url = f"https://krom.one/api/v1/calls?limit={BATCH_SIZE}&offset={offset}"
...
def main():
    ...
    print("=" * 60)
    print("KROM Calls Bulk Downloader (Fixed)")
    print("=" * 60)
    ...
    try:
        while True:
            ...
            # Fetch batch
            print(f"\nBatch {batch_count}: Fetching {BATCH_SIZE} calls (offset={offset})...")
            
            calls = get_krom_calls_batch(offset)
            
            if not calls:
                consecutive_empty += 1
                if consecutive_empty >= 3:
                    print("No data received 3 times. Stopping.")
                    break
            ...
            # Check if we got less than requested (means we might be at the end)
            if len(calls) < BATCH_SIZE:
                print(f"\nGot only {len(calls)} calls (less than {BATCH_SIZE}). Might be near the end.")
