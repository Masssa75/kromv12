#!/usr/bin/env python3
"""
Fixed Current Price Fetcher - Skip tokens we've already tried
Only fetch tokens that:
1. Have a price_at_call (so we know they're not dead)
2. Don't have a current_price yet OR need updating
"""
...
def get_tokens_to_update(limit=50):
    ...
    # Focus on tokens that:
    # 1. Have price_at_call > 0 (not dead)
    # 2. Either have no current_price OR haven't been updated in the last hour
    # 3. Have contract and network info
    
    # Simple query - get tokens with price_at_call > 0 and null current_price
    query = f"""
...
def fetch_from_geckoterminal(contract_address, network):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"success": False, "error": "Token not found (likely delisted since historical price)"}
        else:
...
def update_token_price(token_id, price_data=None, failed=False):
    """Update token's current price and related fields"""
    if failed:
        # Just update the timestamp so we don't keep retrying
        update_data = {
...
def main():
    print("🚀 Fixed Current Price Fetcher")
    print("📊 Only processing tokens with price_at_call > 0\n")
    
    # Get initial statistics
    total_query = f"{SUPABASE_URL}/rest/v1/crypto_calls?select=id&contract_address.not.is.null&network.not.is.null&price_at_call.gt.0"
    ...
    # Count how many already have current prices
    has_price_query = f"{SUPABASE_URL}/rest/v1/crypto_calls?select=id&current_price.not.is.null&current_price.gt.0"
    has_price_resp = requests.get(has_price_query, headers={**headers, "Prefer": "count=exact"}, params={"limit": 0})
    already_done = int(has_price_resp.headers.get('content-range', '/0').split('/')[-1])
    
    print(f"📊 Total eligible tokens (price_at_call > 0): {total_eligible}")
    print(f"✅ Already have current prices: {already_done}")
    print(f"📈 Need to fetch: {total_eligible - already_done}\n")
    ...
    while True:
        ...
        for i, token in enumerate(tokens, 1):
            ...
            if result["success"] and result["current_price"]:
                ...
                if "delisted" in error or ("not found" in error and price_at_call > 0):
                    print(f" 📉 Delisted since historical price")
                    delisted += 1
                    # Update timestamp to avoid retrying
                    update_token_price(token_id, failed=True)
                else:
                    print(f" ❌ {error}")
                    failed += 1
                    # Update timestamp to avoid retrying
        ...
        print(f"\n📊 Batch Complete: {successful} successful, {failed} failed, {delisted} delisted")
        print(f"🔄 Total Processed: {total_processed} | Overall: {already_done + successful}/{total_eligible} ({((already_done + successful)/total_eligible*100):.1f}%)")
    ...
    print(f"📉 Delisted tokens: {delisted}")
    print(f"❌ Other failures: {failed - delisted}")
