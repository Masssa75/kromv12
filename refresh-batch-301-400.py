...
print("=== BATCH PRICE REFRESH (Tokens 301-400) ===")
print("Processing tokens 301-400...\n")

# Get tokens 301-400
tokens = supabase.table('crypto_calls').select(
    'krom_id, ticker, contract_address, network, price_at_call'
).not_.is_('current_price', 'null').not_.is_('contract_address', 'null').order('price_updated_at', desc=False).range(300, 399).execute()
