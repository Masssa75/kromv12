...
print("=== IMPROVED BATCH PRICE REFRESH (100 tokens) ===")
print("Now with proper GeckoTerminal fallback for all missing tokens\n")

# Get 100 tokens that haven't been updated recently (skip the first 100 we already did)
tokens = supabase.table('crypto_calls').select(
    'krom_id, ticker, contract_address, network, price_at_call'
).not_.is_('current_price', 'null').not_.is_('contract_address', 'null').order('price_updated_at', desc=False).range(100, 199).execute()

print(f"Processing tokens 101-200 (oldest update times)...\n")
