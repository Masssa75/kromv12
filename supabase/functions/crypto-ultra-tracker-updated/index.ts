  ...
  for (const token of batch) {
    try {
      ...
      if (updateError) {
        ...
        // Track token revival
        if (token.is_dead && !updateData.is_dead) {
          console.log(`Token ${token.ticker} revived! Liquidity: $${liquidityUsd.toFixed(2)}`)
          revivals.push({ ticker: token.ticker, liquidity: liquidityUsd })
        }
        // Track token death due to low liquidity
        if (!token.is_dead && updateData.is_dead) {
          console.log(`Token ${token.ticker} marked as dead due to low liquidity: $${liquidityUsd.toFixed(2)}`)
          liquidityDeaths.push({ ticker: token.ticker, liquidity: liquidityUsd })
        }
      }
    } catch (error) {
...
serve(async (req) => {
  ...
  try {
    ...
    // Include dead tokens to check for revival (liquidity > $1000)
    let allTokens: any[] = []
    let offset = 0
    const pageSize = 1000
    ...
    while (allTokens.length < maxTokens) {
      const { data: batch, error: fetchError } = await supabase
        ...
        .eq('is_invalidated', false)  // Skip invalidated, but include dead tokens for revival check
    ...
    const deadTokens = tokens.filter(t => t.is_dead).length
    const aliveTokens = tokens.length - deadTokens
    console.log(`Found ${tokens.length} tokens to process (${aliveTokens} alive, ${deadTokens} dead)`)
