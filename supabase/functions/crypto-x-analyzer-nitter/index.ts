...
serve(async (req) => {
  try {
    ...
    if (!anthropicApiKey || !scraperApiKey) {
    ...
    for (const call of unanalyzedCalls || []) {
      try {
        ...
        // Use ScraperAPI to fetch Nitter data - use 'top' filter for highest engagement tweets
        const targetUrl = `https://nitter.net/search?q=${contractAddress}&f=top`
        ...
        // Extract tweet content from Nitter HTML with improved parsing
        ...
        for (const match of tweetMatches) {
          ...
          if (content && content.length > 30 && !content.match(/^[a-zA-Z0-9]{40,}$/)) {
            // Skip tweets that are just contract addresses
            tweets.push({ 
              text: content,
          ...
          if (tweets.length >= 15) break // Get more tweets for better selection
        }

        // Sort by length to prioritize more substantial tweets
        tweets.sort((a, b) => b.length - a.length)
        ...
        if (tweets.length === 0) {
          ...
          // Prepare tweets for Claude analysis - use top 10 highest quality tweets
          const topTweets = tweets.slice(0, 10)
          const tweetContent = topTweets.map((tweet, index) => 
