...
Deno.serve(async (req) => {
  ...
  try {
    // Parse request body
    // Most tokens use 1-2 API calls, new tokens use 3 calls (30 req/min supports ~25 tokens)
    ...
    const { data: tokens, error: fetchError } = await supabase
      ...
      .not('is_dead', 'is', true)
      .not('is_invalidated', 'is', true)  // Skip manually invalidated tokens
      // Order by oldest checked first to cycle through all tokens
    ...
    for (const token of tokens) {
      try {
        ...
        // Check for discrepancy - if calculated ATH is significantly lower than stored
        const discrepancyThreshold = 0.5 // 50% difference
        const storedATH = token.ath_price || 0
        const calculatedATH = athResult.ath_price
        
        if (storedATH > 0 && calculatedATH > 0) {
          const percentDifference = Math.abs(storedATH - calculatedATH) / storedATH
          
          // If calculated ATH is significantly lower than stored ATH
          if (calculatedATH < storedATH && percentDifference > discrepancyThreshold) {
            console.log(`⚠️ DISCREPANCY FOUND for ${token.ticker}: Stored ATH $${storedATH} vs Calculated ATH $${calculatedATH}`)
            
            // Send notification about the discrepancy
            const telegramBotToken = Deno.env.get('TELEGRAM_BOT_TOKEN_ATH')
            const telegramChatId = Deno.env.get('TELEGRAM_GROUP_ID_ATH')
            
            if (telegramBotToken && telegramChatId) {
              const message = `⚠️ *ATH DISCREPANCY DETECTED*\n\n` +
                `Token: ${token.ticker}\n` +
                `Network: ${token.network}\n` +
                `Stored ATH: $${storedATH.toFixed(8)}\n` +
                `Calculated ATH: $${calculatedATH.toFixed(8)}\n` +
                `Difference: -${(percentDifference * 100).toFixed(1)}%\n` +
                `Old ROI: ${token.ath_roi_percent?.toFixed(0)}%\n` +
                `New ROI: ${athResult.ath_roi_percent.toFixed(0)}%\n\n` +
                `The stored ATH appears to be incorrect and has been corrected.`
              ...
              }).catch(err => console.error('Failed to send discrepancy notification:', err))
            }
          }
        }
