...
serve(async (req)=>{
  try {
    ...
    // Step 1: Poll for new calls and trending tokens in parallel
    console.log('Step 1: Polling for new calls and trending tokens...');
    const pollerStart = Date.now();
    const [pollerResponse, geckoTrendingResponse] = await Promise.all([
      fetch(`${supabaseUrl}/functions/v1/crypto-poller`, {
        method: 'POST',
        headers
      }),
      fetch(`${supabaseUrl}/functions/v1/crypto-gecko-trending`, {
        method: 'POST',
        headers
      })
    ]);
    ...
    const geckoTrendingResult = await geckoTrendingResponse.json();
    const pollerTime = Date.now() - pollerStart;
    console.log(`Poller completed in ${pollerTime}ms:`, pollerResult);
    console.log(`GeckoTerminal trending completed:`, geckoTrendingResult);
    ...
    // Step 3: Website analysis (NEW - added to original orchestrator)
    ...
    return new Response(JSON.stringify({
      ...
      results: {
        poller: pollerResult,
        geckoTrending: geckoTrendingResult,
